...
////            Node<E> newNode = new Node(newLaptop, null);
...
//        }
        ...
        if (xPrice < 100.0){
            Node newNode = new Node(new Laptop(xName, xPrice, xColor), null);//???
            if (isEmpty()){
                front = rear = newNode;
            } else {
                rear.setNext(newNode);
    ...
    void updateQueue(){
        ...
        while (p != null){
            if (isPrime(p.getInfo().getColor())){
                ...
                p.getInfo().setPrice(p.getInfo().getPrice() + 20.0);
            }
            p = p.getNext();
        }
        }
    ...
    */   
    Laptop dequeue(){
        ...
        size--;
        return removeNode.getInfo(); // modify the return value appropriately
    ...
    */
    void reverse(){
        ...
        Node next = null;
        while (p != null){
            next.setInfo(p.getNext().getInfo());
//            p.setNext(pre.getInfo());
            pre.setInfo(next.getInfo());
